...
A comprehensive proxy validation tool supporting HTTP, SOCKS4, and SOCKS5 proxies
Enhanced with progress tracking and improved console output
"""
...
╚═╝  ╚═╝╚══════╝╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝    ╚═════╝ ╚══════╝  ╚═══╝  
{Style.RESET_ALL}
{Fore.MAGENTA}                    Proxy Checker Tool v1.2{Style.RESET_ALL}
{Fore.YELLOW}                  Created by Alpha Dev Team{Style.RESET_ALL}
    ...
    def check_http_proxy(self, proxy):
        ...
        try:
            ...
            with self.lock:
                ...
                progress = f"({self.current_progress}/{self.total_proxies})"
                
                if response.status_code == 200:
                    self.good_proxies.append(proxy)
                    print(f"{Fore.GREEN}GOOD: {proxy} {progress}")
                    return True
                else:
                    self.bad_proxies.append(proxy)
                    print(f"{Fore.RED}BAD: {proxy} {progress}")
                    return False
    ...
    def save_results(self, proxy_type, good_proxies, bad_proxies):
        ...
        with open(summary_file, 'w') as f:
            ...
            f.write(f"{'='*50}\n")
            f.write(f"Alpha Dev Proxy Checker v1.2\n")
    ...
    def check_proxies(self, proxy_type, proxies):
        ...
        print(f"{Fore.YELLOW}Total proxies to check: {self.total_proxies}")
